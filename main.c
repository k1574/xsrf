...
static void mediaOpenExternPlayer(Client *c, const Arg *a, WebKitHitTestResult *h);
static void linkOpenExternPlayer(Client *c, const Arg *a, WebKitHitTestResult *h);
...
usage(void)
{
	die("usage: %s [-bBdDfFgGiIkKmMnNpPsStTvwxX]\n"
...
setup(void)
{
	...
	if (signal(SIGHUP, sighup) == SIG_ERR)
		die("%s: Can't install SIGHUP handler.\n", argv0);

	if (!(dpy = XOpenDisplay(NULL)))
		die("%s: Can't open default display.\n", argv0);
...
buildpath(const char *path)
{
	...
	if (g_mkdir_with_parents(apath, 0700) < 0)
		die("%s: Could not access directory: %s\n", argv0, apath);
...
setcert(Client *c, const char *uri)
{
	...
	if (!(cert = g_tls_certificate_new_from_file(file, NULL))) {
		fprintf(stderr, "%s: Could not read certificate file: %s\n", argv0, file);
		return;
	}
...
setstyle(Client *c, const char *file)
{
	...
	if (!g_file_get_contents(file, &style, NULL, NULL)) {
		fprintf(stderr, "%s: Could not read style file: %s\n", argv0, file);
		return;
	}
...
readpipe(GIOChannel *s, GIOCondition ioc, gpointer unused)
{
	...
	if (g_io_channel_read_chars(s, msg, sizeof(msg), NULL, &gerr) !=
	    G_IO_STATUS_NORMAL) {
		fprintf(stderr, "%s: Error reading pipe: %s\n",
		        argv0, gerr->message);
		g_error_free(gerr);
		return TRUE;
	}
	if ((msgsz = msg[0]) < 3) {
		fprintf(stderr, "%s: message too short: %d\n", argv0, msgsz);
		return TRUE;
	}
...
void
mediaOpenExternPlayer(Client *c, const Arg *a, WebKitHitTestResult *h)
{
	Arg arg;
...
void
linkOpenExternPlayer(Client *c, const Arg *a, WebKitHitTestResult *h)
{
	Arg arg;
